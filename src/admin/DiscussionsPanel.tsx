    ...
    <section aria-label="Dyskusje">
      ...
      <div className="mt-3 space-y-2">
        {visible.map((discussion) => {
          const open = openId === discussion.id;
          ...
          return (
            ...
            >
              <button
                type="button"
                onClick={() => setOpenId(open ? null : discussion.id)}
                aria-expanded={open}
                ...
                <div className="border-t border-edge p-3">
                  ...
                  <div className="mt-3 space-y-2">
                    {discussion.messages.map((message, index) => (
                      <div
                        key={index}
                        className="rounded-lg bg-raised p-2.5"
                      >
                  ...
                  <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-edge pt-3">
                    ...
                    <Button
                      size="sm"
                      variant="ghost"
                      icon={discussion.closed ? 'undo' : 'tick'}
                      onClick={() =>
                        setDiscussionClosed(discussion.id, !discussion.closed)
                      }
                    >
                      {discussion.closed ? 'Otwórz ponownie' : 'Ustalone'}
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
